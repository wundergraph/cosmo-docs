...
Any subgraph that resolves or references an entity must declare `@key` with the same set of identifying fields. The router uses these keys to construct _representations_ when fetching entity data from subgraphs that contribute additional fields.
...
[`@external`](/federation/directives/external) marks fields as resolved by another subgraph, making them available for use in `@requires` or `@provides`. [`@requires`](/federation/directives/requires) declares that an entity field depends on external fields that must be fetched from another subgraph first. [`@provides`](/federation/directives/provides) declares that a subgraph can resolve certain external entity fields at a specific query path.
