...
- `@provides` can only be applied to a field that returns an entity type or a list of an entity type.
...
Do not use `@provides` just to avoid declaring `@external`. The field must genuinely be available at that path.
