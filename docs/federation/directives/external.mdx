...
It indicates that the field’s value is resolved by another subgraph, sometimes unconditionally and sometimes only in specific query paths.
...
**`@requires` and `@provides`**:
Only leaf fields (or their ancestors within the same FieldSet)  that are declared in the subgraph but resolved in another must be marked `@external`.
...
In Federation v1, an `@external`  field that is referenced by a `@key(fields: ...)` FieldSet on an extension definition must be explicitly marked.
