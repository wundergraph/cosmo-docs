...
description: "The @external directive marks a field as resolved by another subgraph, making it available for use with @requires and @provides."
---
...
The `@external` directive tells the router that a field is not resolved by the current subgraph. It is defined and resolved by another subgraph, and even though it is declared locally, the current subgraph cannot independently resolve it.
...
- An interface contract, where the field is needed to satisfy the interface but is resolved by another subgraph.
