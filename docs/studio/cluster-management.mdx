...
<Info>Requires a router with version >= 0.66.1</Info>
...
<Note>
  If your router fails to push uptime metrics for any reason, the instance will
  disappear. As long as one metric sample reaches us within 45 seconds, we
  assume the router is operational.
</Note>
