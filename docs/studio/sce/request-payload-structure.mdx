...
| [`labels`](#labelinfo)                 | Contains the labels used to trigger the check. This field will not be present when `subgraphs` is                                            |
...
| [`subgraphs`](#subgraphcontext)        | A list of all the subgraphs that triggered the check. This field is only included when the check is for an existing subgraph.                |
