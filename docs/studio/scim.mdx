...
<CardGroup>
  <Card title="Create users" icon="square-1">
    When a user is added to the SCIM app, it creates the user on Cosmo and then
    adds them to their respective organization.
  </Card>
  <Card title="Update user attributes" icon="square-2">
    When the user's attributes (who is a member of the scim app) are modified,
    it syncs it with Cosmo.
  </Card>
  <Card title="Deactivate users" icon="square-3">
    When a user is removed from the SCIM app, it deactivates the respective
    user's account on Cosmo.
  </Card>
</CardGroup>
