...
* [`router_http_request_duration_milliseconds`](/router/metrics-and-monitoring/prometheus-metric-reference#router_http_request_duration_milliseconds_-sum-count): Measures average request duration to detect latency issues. The maximum duration that can be tracked is 10 seconds, so requests taking longer than will still be counted as taking 10s.
...
**Description:** The request duration in milliseconds. The maximum duration that can be tracked is 10 seconds, so requests taking longer than will still be counted as taking 10s.
