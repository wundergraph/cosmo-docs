...
In this case, the final value used will be `/updatedPath` because the YAML configuration overrides the environment variable.
...
<Warning>
All configuration values do not have an environment var assigned to it. If you require to set something by env vars, but it does not exist, do not hesitate to get in touch with us.
</Warning>
