...
By default, the MCP server runs in stateless mode (`session.stateless: true`). This default avoids scalability issues in load-balanced/serverless environments where sticky sessions is not respected. In stateless mode, the server does not keep any session data but this comes with limitations.
...
- By specification, clients must send a stable `Mcp-Session-Id` value for every new session
