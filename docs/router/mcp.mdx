...
mcp:
  ...
  server:
    ...
    base_url: "http://localhost:5025/mcp" # Optional: Advertised MCP base URL for SSE clients
...
By default, the MCP server runs in stateless mode (`session.stateless: true`). This avoids scalability issues in load-balanced/serverless environments where sticky sessions is not respected. In stateless mode, the server does not keep any session data but this comes with limitations.

When the MCP server runs in stateless mode, capabilities that depend on long-lived connections or server-maintained session context are unavailable or limited:

- No server-initiated notifications or sampling
- No elicitation flows
- No persistent session context or connection resumability
- No bidirectional messaging (request–response only)
- No resource subscriptions

### Stateful mode (sticky sessions required)

If you set `session.stateless: false`, the MCP server maintains per-session state. To ensure a session is consistently handled by the same Router instance, your load balancer or reverse proxy must enforce sticky sessions based on the `Mcp-Session-Id` header:

You need to configure your load balancer to create sticky sessions based on the server's response header `Mcp-Session-Id`, provided the client includes that value in subsequent requests as a request header.

For more information on sticky sessions, see your load balancer or reverse proxy documentation e.g., [F5 NGINX Plus](https://community.f5.com/kb/technicalarticles/mcp-session-affinity-with-f5-nginx-plus/341961).

### Session Storage

In the future, we will evaluate the possibility to add a persistent session storage like Redis to the MCP server. This would allow the server to maintain session state between requests and provide more features like session-based security, resource subscriptions, and bidirectional messaging.
...
Click View > Command Palette > "MCP: Add Server" and use the URL `http://localhost:5025/mcp` to complete the configuration. For more information, see the [MCP Serversn](https://code.visualstudio.com/docs/copilot/chat/mcp-servers).
