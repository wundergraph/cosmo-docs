...
The MCP server respects the "Authorization" header from MCP clients and forwards them to the router. Currently, this functionality is supported by Claude and other tools using the `mcp-remote` command with specific header configuration:
...
When your Cosmo Router is secured using native authentication strategies such as JWT, the GraphQL operations exposed via MCP won't be executable without valid authentication credentials. This ensures that your API remains secure even when the MCP server is enabled. The MCP server honors all authentication and authorization mechanisms configured in your router, so there's no additional security vulnerability introduced by enabling MCP functionality.
