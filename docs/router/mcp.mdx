...
<Note>
  We support only the latest MCP specification with Streamable HTTP support.
</Note>
...
By default, the MCP server runs in stateless mode (`session.stateless: true`). This default avoids scalability issues in load-balanced/serverless environments where sticky sessions may not be respected. In stateless mode, the server does not keep any session data but this comes with limitations.

### Stateful operation (sticky sessions required)
...
- Ensure clients send a stable `Mcp-Session-Id` value for the duration of a session
- Configure your infrastructure to route all requests with the same `Mcp-Session-Id` to the same Router instance

<Warning>
  When running in stateful mode without sticky sessions, features that rely on
  session continuity will behave unpredictably because different requests may be
  routed to different Router instances.
</Warning>

### Features missing in stateless mode with Streamable HTTP
...
- No SSE progress streaming in stateless mode
- No resource subscriptions
- No session-based security (use per-request tokens)
