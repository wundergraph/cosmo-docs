...
<Warning>
  We strongly recommend only using one algorithm per header across your graphs.
  Having different algorithms defined for the same header for different
  subgraphs may result in inconsistent behavior. If there is a serious need for
  that, open an issue in the project.
</Warning>
...
<Note>
  Go canonicalizes headers by default e.g. `x-my-header` to `X-My-Header.` Write
  your rule accordingly or use `(?i)``^X-Test-.*` flags to make your regex case
  insensitive.
</Note>
