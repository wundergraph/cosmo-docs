...
The Cosmo Router supports extending its functionality with gRPC plugins, allowing you to implement subgraphs directly within the router process. This architecture provides significant performance benefits by reducing network hops while maintaining clean separation of concerns.
...
<CardGroup cols={2}>
  <Card title="Higher Performance" icon="bolt">
    Reduce network hops by running resolvers inter-process with the router
  </Card>
