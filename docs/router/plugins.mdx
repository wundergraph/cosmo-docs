---
title: 'gRPC Plugins'
description: 'Extend the Cosmo Router with high-performance gRPC plugins to implement subgraphs directly within the router process or to bridge non-GraphQL services into your federated graph'
icon: "plug"
---
...
gRPC plugins are extensions that run directly inside the Cosmo Router process. They implement the GraphQL resolvers using Google's Remote Procedure Call (gRPC) protocol, providing:
...
4. **In-Process Execution**: Plugins resolve their portion of the query within the router process
...
gRPC plugins for the Cosmo Router provide a powerful way to implement high-performance subgraphs directly within the router process. By eliminating network hops while maintaining a clean architecture, they offer the best of both worlds for many use cases.
