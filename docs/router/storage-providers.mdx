...
<Check>
  By removing the dependency on Cosmo Cloud, your router no longer relies on us
  for operations while still benefiting from all its features.
</Check>
...
<Info>
  **secure** has to be set to `true` when you point to an S3 that uses
  **https\://**
</Info>
