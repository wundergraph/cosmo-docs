...
The `OnSubscriptionStart` handler is a custom module hook that allows you to intercept and customize the initialization of GraphQL subscriptions.
...
In order to use the `OnSubscriptionStart` handler you need to create a [Custom Module](../../custom-modules) which implements the `SubscriptionOnStartHandler` interface.
...
When you return an error from the `OnSubscriptionStart` handler, the router responds to the client with an error event and closes the subscription.
...
<Warning>
Errors are not logged automatically by the router. If you need the error to be logged, you can use the `ctx.Logger()` to log the error yourself.
</Warning>
...
The following example demonstrates how to register a passive `OnSubscriptionStart` handler that logs subscription attempts but allows all subscriptions to proceed normally.
...
// OnSubscriptionStart logs subscription attempts and allows them to proceed
func (m *SubscriptionStartModule) OnSubscriptionStart(
    ctx core.SubscriptionOnStartHandlerContext,
) error {
    logger := ctx.Logger()
    
    // Log subscription details
    if config := ctx.SubscriptionEventConfiguration(); config != nil {
        ...
        logger.Info("Engine subscription started",
...
```go
func (m *CosmoStreamsModule) OnSubscriptionStart(
	ctx core.SubscriptionOnStartHandlerContext,
) error {
	...
	if success {
		ctx.Logger().Debug("Initial event sent to subscriber")
	} else {
