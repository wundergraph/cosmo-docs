...
<Note>
The Handler has two return fields: `events` and `error`.
Whenever an `error` is returned the current bevaviour is that returned `events` are sent to the current subscription client
followed by a normal (non-error) close of the subscription. The error gets logged by the router but it won't be send to the client.
