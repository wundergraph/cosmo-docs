...
We also provide lower level metrics which helps track connection and pool metrics. This can be enabled for Open Telemetry or Prometheus via
...
* `router.http.client.connection.max`: Static configuration values with a subgrpah dimension.

* `router.http.client.connection.active`: The number of connections active for every `subgraph-host` configuration. `active` means that the connection has been initizlied by doing a DNS lookup, TLS handshake and dialing the host.

* `router.http.client.connection.acquire_duration`: The duration that a connection took to be initialized, which includes all of DNS, TLS Handshakes, and Dialing the host.
