...
<Tabs>
  ...
  <Tab title="Connection">
       ...
       We use `server_address`, `server_port` and `subgraph` as dimensions. The `subgraph` dimension is used because it is possible for two subgraphs have the same host and port in the routing url.
