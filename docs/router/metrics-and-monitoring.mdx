...
When [cost analysis](/router/security/cost-control) is enabled and the `cost_stats` toggles are set, the following metrics are emitted:
...
See [Cost Analysis](/router/security/cost-control#telemetry) for full configuration details.
