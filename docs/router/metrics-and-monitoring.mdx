...
* `router.http.request.duration_milliseconds`: End-to-end duration of incoming requests in (histogram). The bucket top boundary is of 10s, i.e., requests taking longer than 10s are counted in the last bucket.
