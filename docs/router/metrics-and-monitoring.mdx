...
* `router_info`: Provides information about the router configuration versions currently in use. There will be one entry for the base configuration and additional entries for each feature flag. For the base configuration, no feature flag name will be attached. The metric value is always set to 1.
...
### HTTP Client Tracing Metrics

The router provides detailed HTTP client tracing metrics that help monitor and debug HTTP requests to subgraphs. These metrics provide insights into various stages of HTTP request processing, including:
...
For a complete list of available HTTP client tracing metrics, see the [HTTP Client Tracing Metrics](/router/http-client-tracing) documentation.
