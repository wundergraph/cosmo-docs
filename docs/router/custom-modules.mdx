...
Note that if you call `Error()` in the hooks, you are most likely to get `nil`, this is as when the hook is called the error has not been set internally yet. The recommended approach is to use a custom response writer, which will be called after the error has been set internally, thus the value from `Error()` being non-nil in error cases. Also do note that `Write` can be called multiple times from the router for the same request.
