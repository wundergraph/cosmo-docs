...
2. **Data Retrieval**: The router first fetches any necessary parent data from relevant gRPC services  
...
The provide better insights of how to implement field resolvers you can take a look at the following Go example:
We would like to retrieve the popularity score for a category based on the id and a given threshold.
...
func (s *MockService) ResolveCategoryPopularityScore(_ context.Context, req *productv1.ResolveCategoryPopularityScoreRequest) (*productv1.ResolveCategoryPopularityScoreResponse, error) {
  // We need to return the result in the original order of the context elements (len(GetContext())).
  ...
  // To keep it simple, in this example we just compare to a base score of 50
  // and either return nil or the base score.
	baseScore := 50
...
<Warning>
Currently a few limitations apply to field resolvers:
- Interfaces and Unions for arguments and context fields are not yet supported.
- Grouping of field resolvers are not yet supported.
</Warning>
