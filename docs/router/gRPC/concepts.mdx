...
graph TD
    ...
    %% Generated Artifacts
    SCHEMA["GraphQL Schema<br/>API Definition"]
    PROTO["Protobuf Schema<br/>gRPC Definition"]
    MAPPING["Mapping Files<br/>Translation Rules"]
    ROUTERFILE["Router Execution File<br/>Complete Config"]
    SERVICE["gRPC Service<br/>Business Logic"]

    SCHEMA --> WGC --> Generated
    ...
    %% Development flow
    Generated --> |"implement"| SERVICE
    Generated --> |"implement"| SERVICE
    Generated --> |"compose"| ROUTERFILE
    Generated --> |"compose"| ROUTERFILE
    
    %% Styling
    classDef devTools fill:#86198f80,stroke:#701a75,stroke-width:4px,color:#ffffff,stroke-dasharray: 10 5
    classDef artifacts fill:#a21caf80,stroke:#86198f,stroke-width:4px,color:#ffffff
    classDef serviceLayer fill:#9d174d80,stroke:#831843,stroke-width:4px,color:#ffffff
    
    class WGC devTools
    class SCHEMA,PROTO,MAPPING,ROUTERFILE artifacts
...
**As a developer**, you start by defining a **GraphQL Schema** with your types and operations. The CLI uses this schema to generate the necessary protobuf definitions and mapping files. 
You can then use the generated protobuf schema to implement your gRPC service.
The generated mapping files are used during composition to configure the router. 
...
graph TD
    %% Client Layer
    CLIENT["Client<br/>GraphQL Operations"]
    
    %% Router Layer
    ROUTER["Cosmo Router"]
    ENGINE["Operation Planning"]
    DATALOADER["Dataloader<br/>Batching"]
    GRPCDATASOURCE["gRPC Plan Execution"]
    
    %% Execution Layer
    PROTOMSG["GraphQL to Proto<br/>(Binary Protocol)"]
    
    %% Service Layer
    SERVICE["gRPC Service<br/>Business Logic"]
    
    %% Runtime flow
    CLIENT --> |"GraphQL Request"| ROUTER
    ROUTER --> ENGINE
    ENGINE --> GRPCDATASOURCE
    GRPCDATASOURCE --> PROTOMSG
    PROTOMSG --> DATALOADER
    DATALOADER --> |"gRPC Call"| SERVICE
    SERVICE --> |"gRPC Response"| DATALOADER
    ROUTER --> |"GraphQL Response"| CLIENT
    
    %% Styling
    classDef clientLayer fill:#ea489980,stroke:#be185d,stroke-width:4px,color:#ffffff,stroke-dasharray: 5 5
    classDef routerLayer fill:#db277780,stroke:#be185d,stroke-width:4px,color:#ffffff
    classDef executionLayer fill:#be185d80,stroke:#9d174d,stroke-width:4px,color:#ffffff
    classDef serviceLayer fill:#9d174d80,stroke:#831843,stroke-width:4px,color:#ffffff
    
    class CLIENT clientLayer
    class ROUTER,ENGINE,DATALOADER,GRPCDATASOURCE routerLayer
    class EXECPLAN,PROTOMSG executionLayer
    class SERVICE serviceLayer
```
