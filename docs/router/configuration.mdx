...
| EXECUTION_CONFIG_STORAGE_IF_NOT_FOUND         | execution_config.storage.if_not_found         | <Icon icon="square" />                        | By default, if the config object is not found, the router will start with a default execution config and continuously poll for the correct configuration. If it is set to error, the router will not start if the execution config is not found. | retry         |
...
| ENGINE_WEBSOCKET_CLIENT_PING_INTERVAL                         | websocket_client_ping_interval                         | <Icon icon="square" /> | The Websocket client ping interval to the subgraph. Defines how often the router will ping the subgraph to signal that the connection is still alive.                                                                                                                                                   | 10s           |
