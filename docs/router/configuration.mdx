...
|                      | condition    | <Icon icon="square" />                        | The retry condition used to decide if a failed subgraph request should be retried.   | `IsRetryableStatusCode() || IsConnectionError() || IsTimeout()` |
