...
| ENGINE_EXPENSIVE_QUERY_CACHE_SIZE                             | expensive_query_cache_size                             | <Icon icon="square" /> | The maximum number of entries in the expensive query cache. This cache protects slow-to-plan queries from being evicted by the main plan cache's LFU policy. Only used when `in_memory_fallback` is enabled. See [Expensive Query Cache](/concepts/cache-warmer#expensive-query-cache).                                                      | 100           |
| ENGINE_EXPENSIVE_QUERY_THRESHOLD                              | expensive_query_threshold                              | <Icon icon="square" /> | The minimum planning duration for a query to be promoted into the expensive query cache. Queries that take longer than this threshold to plan are considered expensive and protected from eviction. The period is specified as a string with a number and a unit, e.g. 10ms, 1s, 5s. The supported units are 'ms', 's', 'm', 'h'.            | 5s            |
