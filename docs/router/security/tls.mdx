...
<Note>
  Per-subgraph TLS configuration uses the same fields nested under `tls.client.all`. A per-subgraph entry fully overrides the global `all` config for that subgraph.
</Note>
