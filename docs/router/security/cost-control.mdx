...
When cost analysis is enabled, the router calculates and records costs for each incoming operation.
...
The router supports both static (estimated) and runtime (actual) cost measurement.
Estimated cost is calculated before execution based on directive weights and list size estimates.
Actual cost is calculated after execution using real list sizes returned by subgraphs.
...
| `max_estimated_limit` | `SECURITY_COST_ANALYSIS_MAX_ESTIMATED_LIMIT` | `0`       | Maximum allowed estimated cost for a query. Only enforced when its value is positive and mode is 'enforce'. |
...
| `expose_headers`      | `SECURITY_COST_ANALYSIS_EXPOSE_HEADERS`      | `false`   | Adds `X-WG-Cost-Estimated` and `X-WG-Cost-Actual` response headers to every operation.                      |
...
func (m *MyModule) Middleware(ctx core.RequestContext, next http.Handler) {
    ...
    if err != nil {
        // Cost analysis was not enabled or the plan is not ready
    }
...
When cost analysis is enabled, the router can emit OpenTelemetry metrics for every operation.
...
| `router.graphql.operation.cost.delta`     | Int64 Histogram | The difference between estimated and actual cost. A positive value means the query was cheaper than estimated. |

<Info>
    Cost metrics require both cost analysis to be enabled (`security.cost_analysis.enabled: true`)
    and the
    corresponding `cost_stats` toggle to be set. The actual and delta metrics additionally require
    execution
...
```
# Average estimated cost per operation
avg by (wg_operation_name) (router_graphql_operation_cost_estimated_sum / router_graphql_operation_cost_estimated_count)
```
