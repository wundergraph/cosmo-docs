...
| `max_estimated_limit` | `SECURITY_COST_ANALYSIS_MAX_ESTIMATED_LIMIT` | `0`       | Maximum allowed estimated cost for a query. Rejection of operatation happens only for positive values and when mode is 'enforce'. |
...
<Info>
    ...
    and the
    corresponding `cost_stats` toggle to be set. The actual metrics additionally require
    execution
