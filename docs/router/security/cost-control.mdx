...
When cost analysis is enabled, the router emits OpenTelemetry metrics for every operation.
...
<Info>
  The estimated and actual cost metrics are emitted for every successfully planned operation, regardless of mode.
  The actual and delta metrics require execution to complete so the router can observe real list sizes.
</Info>
...
# Operations where actual cost exceeds the estimate
router_graphql_operation_cost_delta > 0
```
