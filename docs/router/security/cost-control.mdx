...
but this should be used to estimate how expensive queries will be as a basis.
...
**`enabled`** — When true, the router calculates static cost for every operation.
This is required to use `static_limit` or to access cost from custom modules.

**`static_limit`** — The maximum allowed cost.
Operations exceeding this limit receive a 400 error before execution.
Set to 0 to calculate costs without enforcing a limit (useful for monitoring).

**`list_size`** — The default assumed size for list fields when no `@listSize` directive is specified.
Should reflect typical list sizes in the API.
...
For those familiar with Apollo Router's demand control feature, here are the key differences:
