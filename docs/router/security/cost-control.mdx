...
| `max_estimated_limit` | `SECURITY_COST_ANALYSIS_MAX_ESTIMATED_LIMIT` | `0`       | Maximum allowed estimated cost for a query. Rejection of operation happens only for positive values and when mode is 'enforce'. |
| `estimated_list_size` | `SECURITY_COST_ANALYSIS_ESTIMATED_LIST_SIZE` | `0`       | Default assumed size for list fields when no `@listSize` directive is specified. Required for cost control to work.             |
...
When specified **on a field returning a list** — the weight of this fields is multiplied by the list size:
...
<Info>
    Cost metrics require both cost analysis to be enabled (`security.cost_analysis.enabled: true`) and the
...
The best practice is to assign to fields as small weights as possible. It is better to keep costs low.
If the estimation gets higher than `10000`, then the value will be lost in the infinite bucket.
Non-scalar fields have the default weight of 1. It is a good idea to pick custom weights close to 1.
