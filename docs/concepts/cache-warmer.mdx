...
The in-memory fallback cache warming feature uses the **plan fallback cache** to preserve query plans across hot config reloads and schema changes, reducing latency spikes during restarts.
...
2. **On eviction**: If the main TinyLFU cache evicts a plan that is in the fallback cache, the query plan won't be recomputed and would simply be served from the fallback cache.

When the router reloads, the fallback cache contents are used to rewarm the cache.

<Note>
When using the in-memory fallback, the first start will still experience a cold start, as there is no prior populated plan fallback cache. *Only subsequent reloads* will benefit from the in-memory fallback.
</Note>
...
**Advantages:**
- **Coverage of expensive queries**: By default, queries with planning times above the threshold (5s) are preserved and warmed on reload, protecting slow-to-plan queries from cold-start latency. Users can lower the threshold to any positive duration (e.g., `plan_fallback_threshold: 100ms`) to capture all queries. Users can also set the duration to 1 nanosecond (`plan_fallback_threshold: 1ns`), this would ensure that all queries are cached in the fallback, and thus would be available to rewarm the cache upon reloads.
...
- **Bounded size with duration-based eviction**: The fallback cache has a configurable maximum size. When full, new entries replace the least expensive (shortest planning duration) entries, ensuring the cache always holds the most expensive queries.
...
The plan fallback cache is a secondary cache that protects these slow-to-plan queries from eviction. It is automatically enabled when `in_memory_fallback` is set to `true`.
