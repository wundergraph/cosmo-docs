---
title: "Proposals"
description: "Manage and review changes to federated graphs using proposals"
...
Proposals in Cosmo provide a structured way to manage, review, and implement changes to federated graphs. They allow teams to propose and evaluate modifications to subgraphs before applying them to production, enabling a collaborative review process and minimizing the risk of introducing errors or breaking changes.
...
- **Collaborate on schema evolution**: Enable team members to review and approve changes
- **Document schema changes**: Keep a historical record of modifications to your graph
- **Enforce governance**: Implement proper change management for your GraphQL architecture
- **Prevent breaking changes**: Identify and address potential issues before they impact users

## Proposal States

Proposals go through various states during their lifecycle:

1. **Draft**: The state when the proposal is created
2. **Approved**: The state when the proposal has been approved by authorized team members
3. **Published**: the state when all the subgraphs which are part of the proposal are published.
4. **Closed**: The state when the proposal has been closed
...
```bash
npx wgc proposal create [name] --federation-graph [federated-graph-name] --subgraph/--new-subgraph/--deleted-subgraph [options]
```
...
```bash
npx wgc proposal update [name] --federation-graph [federated-graph-name] --subgraph/--new-subgraph/--deleted-subgraph [options]
```
...
The proposal configuration allows you to set severity levels for subgraph checks and publishes, this is to make sure that the schema used for checks/publishes are a part of a approved proposal.
...
The Schema Comparison view displays a side-by-side diff of the current and proposed schemas. This diff view makes it easy to identify and understand proposed schema modifications, such as the addition of enum values, type changes, or directive modifications.
...
Cosmo automatically executes a comprehensive suite of validation checks whenever a proposal is created or updated. These checks ensure the proposed schema changes are valid, compatible, and adhere to best practices before they can be approved.
...
2. Select one of the following actions:
   - **Approve** - Accept the proposal and apply the schema changes
   - **Close** - Reject the proposal without implementing the changes
