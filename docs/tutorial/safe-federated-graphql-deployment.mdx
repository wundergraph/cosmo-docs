...
All following examples in this guide follow the **dynamic configuration approach** for simplicity. If you're using static configuration, you'll need to modify the examples to include router config building and deployment steps.
...
- Easy rollback by routing traffic away from canary environment or rolling entire subgraph back to the previous version
