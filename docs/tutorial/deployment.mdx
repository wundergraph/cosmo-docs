---
title: "Safe Federated GraphQL Deployment"
description: "A practical guide to safely deploying and managing subgraphs in a federated GraphQL architecture with WunderGraph Cosmo."
icon: "rocket"
---
...
If `wgc subgraph check` fails, fix the issues before proceeding. A failed check means your changes could break the entire graph or existing client applications.
</Warning>
...
GRAPH_API_TOKEN: "your-cosmo-token"
```
...
- ❌ Schema must be accessible to subgraph for embedding (if using post-deployment publishing)
...
- name: Build Router Config
  run: |
    ...
    # Build router image with embedded config
    docker build \
...
- ❌ Must redeploy router for every schema change
- ❌ More complex rollback procedures

---

All examples in this guide follow the **dynamic configuration approach** for simplicity. If you're using static configuration, you'll need to modify the examples to include router config building and deployment steps.
...
Remember: Always deploy your service code first, verify it's healthy, then publish the schema.
...
A canary deployment in federated GraphQL is more complex than typical services because:

- **Schema dependencies**: Your new subgraph version may require other subgraphs to be updated as well
- **Coordinated updates**: Both subgraph and router may need updates
- **Rollback complexity**: Schema rollbacks affect the entire graph
...
- Easy rollback by routing traffic away from canary environment or rolling back to the previous version
...
If the `postPromotionAnalysis` step fails (for example, if schema publishing fails due to composition errors), Argo Rollouts will automatically roll back the deployment to the previous stable version. This ensures that:
