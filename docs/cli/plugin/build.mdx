...
```bash
wgc plugin build [options] [directory]
```
...
| `--debug` | Build the binary with debug information | `false` |
...
```bash
wgc plugin build ./my-plugin
```

### Generate code only

```bash
wgc plugin build --generate-only ./my-plugin
```

### Build for multiple platforms

```bash
wgc plugin build --platform darwin-arm64 linux-amd64 ./my-plugin
```

### Build for all supported platforms

```bash
wgc plugin build --all-platforms ./my-plugin
```
