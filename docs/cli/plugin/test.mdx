...
```bash
wgc plugin test [options] [directory]
```
...
When you initialize a new plugin with `wgc plugin init`, a test file is automatically created at `src/main_test.go`. This file includes a basic test setup that you can expand with your own test cases.
