...
```bash
wgc router plugin init [options] <n>
```
...
2. Generate the Go code with `wgc router plugin build --generate-only ./my-plugin`
3. Implement your resolvers in `src/main.go`
4. Build your plugin with `wgc router plugin build ./my-plugin`
5. Test your plugin with `wgc router plugin test ./my-plugin`
