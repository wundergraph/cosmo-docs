...
```mermaid
flowchart TB
    A[1\. Define Contracts] --> B[2\. Generate & Distribute SDKs]
    B --> C[3\. Configure Router]
    ...
    click B "/connect/api-consumption/produce-generate-distribute-sdks" "Go to Generate & Distribute SDKs"
    click C "/connect/api-consumption/produce-configure-router" "Go to Configure Router"
...
2. **Generate &amp; Distribute SDKs**: Generate type-safe client SDKs (in languages like Go and TypeScript) and OpenAPI specifications from your definitions, ready for distribution.

3. **Configure Router**: Load the proto definitions into the Cosmo Router, which handles protocol translation automatically without server-side code.
